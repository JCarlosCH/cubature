...
#include <limits.h>
#include <float.h>
#include "parametros_del_problema.hpp"
#include "evaluaciones.hpp"
#include "evolucion_temporal.hpp"
...
static int alloc_rule_pts_mod(rule *r, unsigned num_regions)
{
  free(r->pts);
  r->pts = r->vals = NULL;
  r->num_regions = 0;
  r->pts = (double *) malloc(sizeof(double) * 
			     (num_regions 
			      * r->num_points * (r->dim + r->fdim ) ) );
  //printf( "alloc_rule_pts_mod: num_regions * r->num_point * r->dim = %u\n", num_regions * r->num_points * r->dim  );
  if (r->fdim + r->dim > 0 && !r->pts)
    {
      printf( "Memoria insuficiente para el número de muestras\n" );
      return FAILURE;
    }
  r->vals = r->pts + num_regions * r->num_points * r->dim;
  r->num_regions = num_regions;
...
static int rule75genzmalik_evalError_mod_first( rule *r_, unsigned fdimf, integrand_v f, void *fdata, unsigned nR, region *R )
{
  /* lambda2 = sqrt(9/70), lambda4 = sqrt(9/10), lambda5 = sqrt(9/19) */
  const double lambda2 = 0.3585685828003180919906451539079374954541;
  const double lambda4 = 0.9486832980505137995996680633298155601160;
  ...
  parametros_del_problema* params = ((parametros_del_problema *) fdata);
  const unsigned int Nt = params->Nt;
  const unsigned int num_samples = Nt + 1;

  unsigned fdim = params->fdimt * num_samples;	//Para asignar, desde ahora, la memoria necesaria para la evolución temporal
  r_->fdim = fdim;
  if (alloc_rule_pts_mod( r_, nR )) return FAILURE;
  pts = r_->pts; vals = r_->vals;

  //printf( "sizeof(pts) = %u\t sizeof(vals) = %u\n", sizeof(&pts), sizeof(&vals) );
  for (iR = 0; iR < nR; ++iR) {
    const double *center = R[iR].h.data;
    const double *halfwidth = R[iR].h.data + dim;
    ...
    free(R[iR].ee);
    R[iR].fdim = fdim;
    R[iR].ee = R[iR].h.data ? (esterr *) malloc(sizeof(esterr) * fdim) : NULL;
  ...
  fdim = fdimf;	//Para regresar a la dimensión original
  double val[ fdim ];
  //printf( "fdim = %u, npts = %lu\n", fdim, npts );
  if (f(dim, npts, pts, fdata, fdim, vals))
    {
      return FAILURE;
    }
...
//  printf("j = %u:", (params->jx2)>>1);
//  for (j = 0; j < fdim; j++){
//    val[j] *= 0.03125 * M_1_PI * M_1_PI;
//    printf( "\t%1.16g", val[j]);//, err[0] );
//  }
//  printf("\n");
  FILE* nFile;
  char nFile_name[95];
  params->archivo_dest(nFile_name,95);
  nFile = fopen( nFile_name, "w" );
  fprintf( nFile, "#Tolerancias de integración: absoluta = %1.1g, relativa = %1.1g. Tolerancias en la solución del SEDO: absoluta = %1.1g, relativa = %1.1g. Nt = %u.\n#Valores inciales", params->ReqAbsError, params->ReqRelError, params->abs_error, params->rel_error, params->Nt);
  for (unsigned j=0; j<fdimf; j++)
    fprintf(nFile, " %1.15g ", val[j]);
  fprintf( nFile, "\n" );
//  for (unsigned j=0; j<fdimf; j++)
//    fprintf( nFile, "\n%s", params->etiquetas[j] );
  fclose( nFile );
  return SUCCESS;
}

static int rule75genzmalik_evalError_mod( rule *r_, unsigned fdimt, integrand_v ft, void *fdata, unsigned nR, region *R )
{
  const double weight2 = 980. / 6561.;
  const double weight4 = 200. / 19683.;
  ...
  unsigned i, j, iR, dim = r_->dim;
  parametros_del_problema* params = ((parametros_del_problema *) fdata);
  const size_t npts = params->npts;
  const double dt = params->dt;
  const unsigned T = static_cast< unsigned >( params->t_f / dt );
  unsigned t_i = static_cast< unsigned int >( params->t_0 / dt );
  const unsigned int Nt = params->Nt;
  const unsigned int num_samples( Nt + 1 );
  //params->set_t_0( t_i * Nt * dt );
  //params->set_t_f( ( t_i + 1 ) * Nt * dt );
  printf( "t_0 = %1.3f, t_f = %1.3f\n", params->t_0, params->t_f );
  double /**diff,*/ *pts, *vals;
  pts = r_->pts;
  vals = r_->vals;
  ...
  double val[ fdimt ];
  FILE* nFile;
  //std::string archivo = "j-discreto/" + j_val_str + "_JJ_ZZ_Valor_Esperado-TWA-k100-jdiscreto-con-desplazamiento-en-Psi.dat";
  char nFile_name[95];
  params->archivo_dest(nFile_name, 95);
  nFile = fopen( nFile_name, "a" );
  ...
  while ( t_i * Nt < T ) {
    ...
    //params->set_t_f( (t_i + 1) * Nt * dt );
    params->t_i = t_i;
    printf("Resolviendo ecuaciones diferenciales\n");
    solve_SEDO( params );
    //printf("Se terminó de resolver las ecuaciones diferenciales\n");
    //printf( "t_0 = %1.3f, t_f = %1.3f\n", params->t_0, params->t_f );
    if (ft(dim, npts, pts, fdata, params->fdimt, vals))
      return FAILURE;

    for (j = 0; j < fdimt; ++j) {
      const double *v = vals + j;
#define VALS(i) v[fdimt*(i)]	       
      for (iR = 0; iR < nR; ++iR) {
	double result; //, res5th;
	double val0, sum2=0, sum3=0, sum4=0, sum5=0;
	unsigned k, k0 = 0;
	...
	result = R[iR].h.vol * (r->weight1 * val0 + weight2 * sum2 + r->weight3 * sum3 + weight4 * sum4 + r->weight5 * sum5);
	R[iR].ee[j].val = result;
	v += r_->num_points * fdimt;
      }	
#	     	undef VALS
    }
    /* Resummation */
    for (j = 0; j < fdimt; ++j) val[j] = 0;
    //#pragma omp parallel for shared( fdimt, nR, R, val ) private( i, j )
    for (i = 0; i < nR; ++i) {
      for (j = 0; j < fdimt; ++j) {
	val[j] += R[i].ee[j].val;
      }
    }
//    for (unsigned kl = 0; kl < fdimt; kl++){
//      printf("%1.16f\n", val[kl]);
//    }
    for ( unsigned m = 0; m < num_samples; m++ ){
      //printf( "\n%5.4f", dt * ( m + t_i * Nt ) );
      fprintf( nFile, "\n%5.6f", dt * ( m + t_i * Nt ) );
      for ( unsigned n = 0; n < fdimt / num_samples; n++ )
	{
	  fprintf( nFile, "\t%1.16g", val[ m * fdimt / num_samples + n ] );
	}
    }
    fflush( nFile );
    //Copiar las condiciones finales a las condiciones iniciales:
    //    for ( unsigned m = 0; m < npts; m++ ){
    //      params->t_pts[ m * 4 + 0 ] = params->t_pts[ ( Nt * npts + m ) * 4 + 0 ];
    //      params->t_pts[ m * 4 + 1 ] = params->t_pts[ ( Nt * npts + m ) * 4 + 1 ];
    //      params->t_pts[ m * 4 + 2 ] = params->t_pts[ ( Nt * npts + m ) * 4 + 2 ];
    //      params->t_pts[ m * 4 + 3 ] = params->t_pts[ ( Nt * npts + m ) * 4 + 3 ];
    //    }
    //    //#pragma omp parallel for shared( params )
    //for ( unsigned m = 0; m < npts; m++ ){
    //	params->t_pts[ npts * ( dim + 1 + 2 ) + m * ( Nt + 1 ) ] = params->t_pts[ npts * ( dim + 1 + 2 ) + m * ( Nt + 1 ) + Nt ];
    //}
    t_i++;//+= Nt;
    //tasks.clear();
  }
...
/* adaptive integration, analogous to adaptintegrator.cpp in HIntLib */
  ...
  static int rulecubature(rule *r, unsigned fdim,
			  integrand_v f, integrand_v f0, integrand_v ft, void *fdata, 
			  const hypercube *h, 
			  size_t maxEval,
...
{
  parametros_del_problema* params = ( ( parametros_del_problema* ) fdata );
  const unsigned Nt = params->Nt;
  //const unsigned npts = params->npts;
  //double* t_pts;// = params->t_pts;
  size_t numEval = 0;
  //size_t NR = 0;
  heap regions;
  unsigned i, j;
  ...
  while (numEval < maxEval || !maxEval) {
    ...
    if (parallel) { /* maximize potential parallelism */
	 ...
	 ggg_Numerical Integration. Recent Developments,
  ...
  for (j = 0; j < fdim; j++)
    printf( "f[%d] = %1.17g +/- %1.17g\n", j, val[j], err[j] );
  //return SUCCESS;
  if (params->t_f > 0.5 * params->dt){
    /*Cálculo de los valores iniciales de la función, los resultados se almacenan en W_vals y se reutilizan en los
     * cálculos con la función W_disc_euler_v
     */
    //fdim = 2;
    //printf("mod_first\n");
    const unsigned fdimf = params->fdimf;

    if ( rule75genzmalik_evalError_mod_first( r, fdimf, f0, fdata, regions.n, regions.items )) goto bad;
    //eval_regions_mod( regions.n, regions.items, W_disc_euler_v_first, fdata, r, fdim );
    //printf( "Número de regiones: %lu\n", regions.n );
    //return SUCCESS;
    const unsigned fdimt = params->fdimt * (Nt + 1);	//Para evaluar en una sola llamada los puntos en cada muestra de tiempo
    if (rule75genzmalik_evalError_mod(r, fdimt, ft, fdata, regions.n, regions.items)) goto bad;
  }
  for (i = 0; i < regions.n; ++i) {
...
static int cubature(unsigned fdim, integrand_v f, integrand_v f0, integrand_v ft, void *fdata, 
		    ...
		    double *val, double *err, int parallel)
{
  ...
  status = !h.data ? FAILURE
    : rulecubature(r, fdim, f, f0, ft, fdata, &h,
...
int hcubature_v(unsigned fdim, integrand_v f, integrand_v f0, integrand_v ft, void *fdata, 
		unsigned dim, const double *xmin, const double *xmax, size_t minEval,
		size_t maxEval, double reqAbsError, double reqRelError, 
		error_norm norm,
		double *val, double *err)
{
  return cubature(fdim, f, f0, ft, fdata, dim, xmin, xmax, 
...
//	d.f = f; d.fdata = fdata;
//	(unsigned fdim, unsigned fdimt, integrand_v f, integrand_v func_first, integrand_v funct, void *fdata, 
//	 ret = cubature(fdim, fdimt, fv, &d, dim, xmin, xmax, 
